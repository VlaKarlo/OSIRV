...
print("Starting robust segmentation polygon label generation...")
        ...
        for tool_folder in gt_path.iterdir():
            ...
            for gray_val, offset in PART_OFFSETS.items():
                ...
                for c in contours:
                    ...
                    # Approximate contour to polygon vertices
                    epsilon = 0.002 * cv2.arcLength(c, True)
                    approx = cv2.approxPolyDP(c, epsilon, True)

                    # YOLO segmentation requires at least 3 points (6 coordinate values)
                    if len(approx) < 3:
                        continue

                    # Format line with class ID followed by normalized polygon x, y vertices
                    polygon_str = f"{final_class_id}"
                    for point in approx:
                        px = point[0][0] / cw
                        py = point[0][1] / ch
                        polygon_str += f" {px:.6f} {py:.6f}"

                    yolo_lines.append(polygon_str + "\n")
...
print(
    f"\nSuccess! Created {global_frame_count} valid segmentation label files in"
    f" {OUTPUT_DIR}"
)